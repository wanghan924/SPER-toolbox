...
% Last Modified by GUIDE v2.5 17-Jul-2021 15:36:51
...
for kk = 1:length(boundaries)
    ...
    if N>5
        ...
        PP = Pattern_extraction([x,y],length(X),XY{kt},R_data{kt});
        Feature{kt} = PP;
        %PR = R_data{kt}(:);
        R_mean = mean(R_data{kt}(:));
        R_total = sum(R_data{kt}(:))*sqrt(handles.res);
%         for ii = handles.threshold:5:max(R_data{kt})
%             ik = ik+1;
%             R1_mean(1,ik) = mean(PR(PR>ii));
%             R1_total(1,ik) = sum(PR(PR>ii))*sqrt(handles.res);
%         end
        TT(kt,1:11) = [kt PP.YCentriod*sqrt(handles.res)*sqrt(handles.res) PP.XCentriod*sqrt(handles.res)*sqrt(handles.res) PP.Area*handles.res PP.MajorAxesAngle PP.sp R_mean R_total PP.core(1) PP.core(2) PP.core(3)];
        X = [];Y = [];kt=kt+1;%PR = [];
    end
end
...
elseif size(XY,1)<1000 && size(XY,1)>500
...
fprintf(fileID,'%80s \r\n','================================================================================');
fprintf(fileID,'%22s \r\n',strcat('ROIs are extracted over ',num2str(handles.threshold)));
fprintf(fileID,'%80s \r\n','================================================================================');
fprintf(fileID,'%51s \r\n\r\n','Pattern features are listed below:');
fprintf(fileID,'%6s %12s %12s %12s %12s %12s %12s %12s %12s %12s %12s\r\n','No','Location (x)','Location (x)','Size','Orientation(w)','Shape(sp)','Areal average','Total volume','Core(x)','Core(y)','Core(areal average)');
